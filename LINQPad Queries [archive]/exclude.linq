...
void Main()
{
    ...
    var response = http.GetStringAsync("http://sport3.betlab.com/spout/api/events").Result;
...
//    var join = (from left in eventsUat 
//                join right in eventsStage on left.Id equals right.Id
//                select new 
//                {
//                    left.Id,  
//                    UatName = left?.Name?.FirstOrDefault(x => x.Lang == "en")?.Text, 
//                    StageName = right?.Name?.FirstOrDefault(x => x.Lang == "en")?.Text
//                }).Where(e =>e?.UatName.Trim() != e?.StageName.Trim()).ToList().Dump();

    var feedEn = eventsProd.Where(e => e.Name?.FirstOrDefault(x => x.Lang == "en") == null)
        .ToList()
        .Dump();
